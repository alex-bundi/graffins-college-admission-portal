...
onMounted(() => {
    ...
    let courseLines = Object.keys(props.applications).forEach(application => {
        const applicationData = props.applications[application];
        Object.keys(applicationData.course_lines).forEach(courseLine => {
            const courseLineData = applicationData.course_lines[courseLine];

            if (courseLineData.payment_updated != 1){
...
const hasPendingPayments = computed(() => {
    Object.keys(props.applications).forEach(application => {
        const applicationData = props.applications[application];
        Object.keys(applicationData.course_lines).forEach(courseLine => {
            const courseLineData = applicationData.course_lines[courseLine];
                ...
                <div class="mt-5">
                    ...
                    <ul class="flex flex-col space-y-6">
                        ...
                        <li  v-for="application in submittedApplications" :key="application.id"
                            class=" flex flex-row items-center w-full sm:w-3/4 justify-between space-x-8 overflow-hidden bg-white rounded-lg shadow-md 
                            border border-amber-500 text-gray-700 transition hover:shadow-lg hover:border-amber-700 p-4">
                            <div class="flex flex-row space-x-5 items-center">
                                    <div class="">
                                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="size-6">
                                        <path d="M11.25 4.533A9.707 9.707 0 0 0 6 3a9.735 9.735 0 0 0-3.25.555.75.75 0 0 0-.5.707v14.25a.75.75 0 0 0 1 .707A8.237 8.237 0 0 1 6 18.75c1.995 0 3.823.707 5.25 1.886V4.533ZM12.75 20.636A8.214 8.214 0 0 1 18 18.75c.966 0 1.89.166 2.75.47a.75.75 0 0 0 1-.708V4.262a.75.75 0 0 0-.5-.707A9.735 9.735 0 0 0 18 3a9.707 9.707 0 0 0-5.25 1.533v16.103Z" />
                                ...
                                <div class="flex flex-row space-x-10 items-center">
                                    <!-- Name -->
                                        <div>
                                            ...
                                            <div class="">
                                                <div v-if="(hasPendingPayments)" class="mt-4 w-full">
                                                    <div>
